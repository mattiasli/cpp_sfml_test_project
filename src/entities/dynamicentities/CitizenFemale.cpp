...
{
    handler.getSpriteManager().getAnimation(constants::EntityType::CitizenFemale, entityState, direction)->getSprite()->setPosition(worldCoordinate);
    handler.getRenderWindowManager().getRenderWindow().draw(*handler.getSpriteManager().getAnimation(constants::EntityType::CitizenFemale, entityState, direction)->getSprite());
}
